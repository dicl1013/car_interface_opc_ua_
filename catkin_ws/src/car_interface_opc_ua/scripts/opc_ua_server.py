...
import sys
sys.path.append("/app/src/car_interface_opc_ua/scripts")
...
class UserManager:
    ...
    def get_user(self, iserver, username=None, password=None, certificate=None):
        """
        Get the user with the given username and password. If no correct user is given, return None
        """
        if username is None:
            return None
            ...
            while not rospy.is_shutdown():
                ...
                if data:
                    ...
                    if data[0] == "PublishData":
                        ...
                        self.CurrentLogInData["Username"] = ''
                        self.CurrentLogInData["Password"] = ''
                        ...
                        if Username != self.CurrentLogInData["Username"] or Password != self.CurrentLogInData["Password"]:
                            # if the current user was not already deleted:
                            if self.CurrentLogInData["Username"] != '':
